  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [dailyData, routeData, vehicleData, statsData] = await Promise.all([
          apiFetchJson<DailyEmission[]>("/footprint/daily/trend"),
          apiFetchJson<RouteEmission[]>("/charts/routes?limit=5"),
        ...
        <div className="relative z-10 container mx-auto px-4 pt-24 pb-16">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard 
              ...
              value={`${stats?.total_co2_today.toFixed(2) || 0} kg`} 
              delta="Real-time data" 
            />
            <StatCard 
              label="Total CO2 This Month" 
              value={`${stats?.total_co2_month.toFixed(2) || 0} kg`} 
              delta="Month to date" 
            />
            <StatCard 
              label="Total Trips" 
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            ...
            <div className="glass-card rounded-2xl p-6 shadow-soft hover-lift border border-emerald/10">
              ...
              <div className="mt-4 rounded-xl bg-emerald/10 px-4 py-3 text-xs font-semibold text-emerald">
                Total CO2: {stats?.total_co2_all.toFixed(2) || 0} kg
              </div>
              <ul className="mt-5 space-y-3 text-sm text-muted-foreground">
                <li>📊 {stats?.total_trips || 0} trips tracked across all routes</li>
                <li>🚛 {stats?.active_vehicles || 0} vehicles active in the last 24 hours</li>
                <li>🌍 {stats?.total_routes || 0} unique routes monitored</li>
              </ul>
            </div>
          </div>
        </div>
